...
test("formatSeasons test", async () => {
  const seasons = formatSeasons(episodes);

  await expect(seasons.hasOwnProperty("Season 1")).toBeTruthy();
  await expect(seasons.hasOwnProperty("Season 2")).toBeTruthy();

  await expect(seasons["Season 1"]).toHaveLength(8);
  await expect(seasons["Season 2"]).toHaveLength(4);
});
