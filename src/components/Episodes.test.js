...
test("render episodes without errors", () => {
  const { rerender, getByText, queryByText, getAllByText } = render(
    <Episodes episodes={[]} />
  );

  expect(queryByText(/season 1, episode 1/i)).toBeNull();
  expect(queryByText(/season 1, episode 2/i)).toBeNull();

  expect(queryByText(/the vanishing of will byers/i)).toBeNull();
  expect(queryByText(/the weirdo on maple street/i)).toBeNull();

  rerender(<Episodes episodes={episodesSeason1} />);
  ...
  expect(getByText(/the weirdo on maple street/i)).toBeInTheDocument();
  ...
  expect(
    getByText(
      /while the search for the missing Will continues, joyce tells jim about a call she apparently received from her son./i
    )
  ).toBeInTheDocument();
