...
test("render app with data loaded", async () => {
  mockFetchShow.mockResolvedValueOnce(showData);

  const { getByText, getAllByText } = await render(<App />);

  expect(getAllByText(/stranger things/i)[0]).toBeInTheDocument();
  expect(getByText(/a love letter to the '80s classics/i)).toBeInTheDocument();
  ...
  await waitFor(() => {
    expect(getByText(/will the wise/i)).toBeInTheDocument();
  });
});
